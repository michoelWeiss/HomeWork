(function () {
    ...
    function UpdateZIndex(e) {
        for (let i = 0; i < objectsOnScreen.length; i++) {
            ...
            else if(e.classList.contains('potatoHead'))
                objectsOnScreen[i].style.zIndex = 1;
        }
    }

    /// Update Location ///
    function UpdateLocation(e, top, left){
        for (let i = 0; i < objectsOnScreen.length; i++) {
            if (
                !e.classList.contains('dropdown-btn') &&
                !e.closest('.dropdown-content') &&
                ...
                objectsOnScreen[i].style.x = top;
                objectsOnScreen[i].style.y = left;
            }
        }
    }
    ...
    function saveState() {
        if (objectsOnScreen.length > 0) {
            ...
            for(let i = 0; i < objectsOnScreen.length; i++){
                const currentItem = {
                    src: objectsOnScreen[i].getAttribute('src'),
                    className: objectsOnScreen[i].className,
                    top: `${objectsOnScreen[i].x}px`,
                    left: `${objectsOnScreen[i].y}px`,
                    zIndex: `${objectsOnScreen[i].style.zIndex}`,
                    ID: objectsOnScreen[i].id
                };


                saveItems.push(currentItem);
                console.log('saveItems', saveItems);
            }
            localStorage.setItem('parts', JSON.stringify(saveItems));
        }
    }
    ...
    function LoadSavedImage(){
        if(localStorage.parts){
            const savedImage = localStorage.parts;
